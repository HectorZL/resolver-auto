...
class ExamAgent:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        # Sistema de reintentos interno por actividad
        # Estructura: {"activity_name": {"attempts": 0, "questions_total": 0, "questions_correct": 0}}
        self.activity_attempts = {}
        
        # Tracking de preguntas repetidas para cambiar modelo
        self.question_repeat_count = {}  # {"question_hash": count}
    ...
    def find_incomplete_module(self, exclude_modules=None) -> dict:
        ...
        try:
            ...
            for idx, module in enumerate(modules):
                ...
                print(f"[DEBUG] Módulo '{title}': {len(spans)} spans encontrados")
                for span in spans:
                    text = span.inner_text()
                    if "%" in text and "completed" in text.lower():
                        progress_text = text
                        print(f"[DEBUG] Módulo '{title}': Progreso encontrado = '{progress_text}'")
                        break
                ...
                if match:
                    ...
                    print(f"[DEBUG] Módulo {idx} '{title}': {status} ({progress}%)")
                    ...
                    if progress < 100:
                        ...
                        # Omitir este print, ya se muestra arriba
                else:
    ...
    def find_incomplete_activity(self, module_element, skip_activities=None) -> dict:
        ...
        print(f"[DEBUG] Actividades a resolver: {self.activities_to_solve}")
        ...
        try:
            ...
            for idx, activity in enumerate(activities):
                ...
                elif name not in self.activities_to_solve:
                    skip_reason = f"⏭️ No está en activities_to_solve"
                elif is_disabled:
                ...
                print(f"[DEBUG] Actividad {idx} '{name}': {height}% | {disabled_text} | {status}")
                all_activities_info.append(f"  - {name}: {height}% ({status})")
                
                # Si hay razón de skip, continuar
                if skip_reason:
                    continue
                
                # Esta es una actividad válida e incompleta
                print(f"\n[INFO] ✅ Seleccionada actividad incompleta: {name} ({height}%)")
                return {
                    "name": name,
                    "progress": height,
                    "element": activity,
    ...
    def solve_current_question(self) -> bool:
        ...
        try:
            ...
            # NUEVO: Detectar preguntas repetidas y cambiar modelo
            question_hash = hash(question_text[:100] if question_text else "")
            self.question_repeat_count[question_hash] = self.question_repeat_count.get(question_hash, 0) + 1
            
            if self.question_repeat_count[question_hash] >= 3 and not self.solver.using_advanced:
                print(f"[WARNING] Pregunta repetida {self.question_repeat_count[question_hash]} veces → Modelo AVANZADO")
                self.solver.switch_to_advanced_model()
            elif self.question_repeat_count[question_hash] < 3 and self.solver.using_advanced:
            ...
            if question_type == "multiple_choice":
                success = self._solve_multiple_choice(question_text)
                if success:
                    self.question_repeat_count[question_hash] = 0
                    if self.solver.using_advanced:
                        self.solver.reset_to_normal_model()
                return success
            elif question_type == "fill_blanks":
                success = self._solve_fill_blanks(question_text)
                if success:
                    self.question_repeat_count[question_hash] = 0
                    if self.solver.using_advanced:
                        self.solver.reset_to_normal_model()
                ...
                success = self._solve_matching_buttons(question_text)
                if success:
                    self.question_repeat_count[question_hash] = 0
                    if self.solver.using_advanced:
                        self.solver.reset_to_normal_model()
...
Sin asteriscos, sin explicaciones, sin formato markdown."""
            ...
            def normalize(text):
                ...
                # Usar códigos Unicode explícitos para evitar corrupción de caracteres
                ...
                normalized = normalized.replace("\u201D", "'")  # U+201D RIGHT DOUBLE QUOTATION MARK → '
                normalized = normalized.replace("\u201C", "'")  # U+201C LEFT DOUBLE QUOTATION MARK → '
                normalized = normalized.replace("\u0060", "'")  # U+0060 GRAVE ACCENT → '
                normalized = normalized.replace("\u00B4", "'")  # U+00B4 ACUTE ACCENT → '
                normalized = normalized.replace("\u02BC", "'")  # U+02BC MODIFIER LETTER APOSTROPHE → '
                normalized = normalized.replace("`", "'")       # Backtick → '
                normalized = normalized.replace("´", "'")       # Acute accent (fallback) → '
            ...
            for line in result_text.split("\n"):
                ...
                if row_num < len(rows_data):
                    ...
                    print(f"[DEBUG] Buscando '{answer}' en opciones: {[normalize(o['text']) for o in row['options']]}")
                    print(f"[DEBUG] HEX answer: {answer.encode('utf-8').hex()}")
                    print(f"[DEBUG] HEX opciones: {[normalize(o['text']).encode('utf-8').hex() for o in row['options']]}")
                    ...
                    for opt in row['options']:
                        ...
                        if opt_norm == answer:
                            matched_opt = opt
                            print(f"[DEBUG] Match exacto encontrado: '{opt_norm}' == '{answer}'")
                            break
                    ...
                    if not matched_opt:
                        for opt in row['options']:
                            ...
                            if answer in opt_norm or opt_norm in answer:
                                matched_opt = opt
                                print(f"[DEBUG] Match por contención: '{opt_norm}' contiene '{answer}'")
                                break
...
"""
    ...
    def _get_max_attempts(self, attempts_done: int) -> int:
        ...
        Límite generoso: 5 intentos totales para todas las actividades.
        """
        return 5  # Permitir hasta 5 intentos para completar cada actividad
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                # Buscar módulo incompleto (excluyendo ya procesados)
                module = self.find_incomplete_module(exclude_modules=processed_modules)
                ...
                # Inicializar set de actividades saltadas si es necesario
                if module_key not in skipped_activities_per_module:
                    skipped_activities_per_module[module_key] = set()
                
                # Buscar actividad incompleta (saltando las ya intentadas sin éxito)
                activity = self.find_incomplete_activity(
                    module["element"],
                ...
                if not activity:
                    ...
                    # Marcar módulo como procesado y continuar
                ...
                # Si ya está marcada como completada, saltarla
                ...
                # Calcular intentos permitidos basado en intentos previos
                attempts_done = self.activity_attempts[activity_key]["attempts"]
                max_attempts = self._get_max_attempts(attempts_done)
                
                # Si excedió los intentos máximos (sin completar), saltar y buscar otra actividad
                if attempts_done >= max_attempts:
                    print(f"[WARNING] Actividad '{activity['name']}' alcanzó {attempts_done} intentos sin completarse (máx: {max_attempts}).")
                    print(f"[INFO] Buscando otras actividades en este módulo...")
                    skipped_activities_per_module[module_key].add(activity['name'])
                    continue
                ...
                # Volver al dashboard y verificar progreso real
                self.browser.sleep(2)
                try:
                    self.browser.page.go_back()
                    self.browser.sleep(1)
                    # Recargar página para visualizar mejor el progreso
                    self.browser.page.reload()
                    self.browser.sleep(2)
                    print("[INFO] Página recargada para actualizar progreso")
                    
                    # Verificar el progreso actual de la actividad
                    module_updated = self.find_incomplete_module(exclude_modules=set())
                    if module_updated and module_updated['title'] == module['title']:
                        activity_updated = self.find_incomplete_activity(module_updated["element"])
                        
                        # Si la actividad ya no aparece como incompleta, está al 100%
                        if not activity_updated or activity_updated['name'] != activity['name']:
                            ...
                            # Actividad aún incompleta, mostrar progreso
                            current_progress = activity_updated.get('progress', 0)
                            print(f"[INFO] Actividad '{activity['name']}' aún incompleta: {current_progress}% ({questions_in_activity} preguntas respondidas)")
                            print(f"[INFO] Se reintentará en el próximo ciclo...")
                    else:
                        # No se pudo verificar, asumir incompleta
